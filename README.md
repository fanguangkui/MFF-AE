...
## 3. Missing Value Imputation Comparison

Compare different missing value imputation methods using the Isolation Forest algorithm for anomaly detection.

### Available Imputation Methods

- **Global Minimum Imputation** (`min`): Fills missing values with the global minimum value of each feature
- **Mean Imputation** (`mean`): Fills missing values with the mean value of each feature
- **Low-Random-Value Imputation** (`low_random`): Fills missing values with low random values
- **Linear Interpolation** (`linear`): Fills missing values using linear interpolation
- **KNN Imputation** (`knn`): Fills missing values using K-Nearest Neighbors

### Run Comparison

``` bash
# Global Minimum Imputation (Original Method)
python train_OD.py -c configs/MissingValueComparison/Standard_AutoEncoder_min.yaml data_dir datasets/Fake train_status true infer_status false eval_status false

# Mean Imputation
python train_OD.py -c configs/MissingValueComparison/Standard_AutoEncoder_mean.yaml data_dir datasets/Fake train_status true infer_status false eval_status false

# Low-Random-Value Imputation
python train_OD.py -c configs/MissingValueComparison/Standard_AutoEncoder_low_random.yaml data_dir datasets/Fake train_status true infer_status false eval_status false

# Linear Interpolation
python train_OD.py -c configs/MissingValueComparison/Standard_AutoEncoder_linear.yaml data_dir datasets/Fake train_status true infer_status false eval_status false

# KNN Imputation
python train_OD.py -c configs/MissingValueComparison/Standard_AutoEncoder_knn.yaml data_dir datasets/Fake train_status true infer_status false eval_status false
```

### Results

Results will be saved in the `Result/MissingValueComparison/` directory, with subdirectories for each imputation method.

Each run will output:
- Detected outlier samples
- Fitting time
- Outlier indices and scores

### Configuration Files

All configuration files are located in `configs/MissingValueComparison/` directory, each corresponding to a different imputation method.

The configurations use Isolation Forest algorithm for anomaly detection with `contamination: 0.03` parameter.
