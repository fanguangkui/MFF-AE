...
class MaskCTRAE2(BaseDetector):
    def __init__(self,
                 ...
                 topk_value=1,
                 device=None):
        super(MaskCTRAE2, self).__init__(contamination=contamination)
    ...
    def _train_autoencoder(self, train_loader):
        ...
        for iname, ipara in self.model.named_parameters():
            ...
            elif "memory" in iname:
                memory_para.append(ipara)
            else:
                AE_para.append(ipara)
        param_groups = [
            {'params': mask_para, 'lr_mult': 10},
        ...
        for epoch in range(self.epochs):
            ...
            for data, data_idx, batch_y_label in train_loader:
                ...
                mask_data = self.model.adaptive_format(data)
                # reconstruction loss
                if "MSE" in self.loss_fn_name:
                ...
                if "mask_loss" in self.loss_fn_name:
                    ...
                    # print(torch.max(self.model.adaptive_format.important_mask), torch.min(self.model.adaptive_format.important_mask))
                    # version 1:
                    ...
                    #     self.model.important_mask.repeat(format_data.size(0), 1) * format_data
                    # )
    ...
    def decision_function(self, X, sample_name=None):
        ...
        mask_outlier_scores = np.zeros([X.shape[0], ])
        with torch.no_grad():
