...
def loss_overlapBAK(score, assign_prob, seed, batch_y_label, bw_u=1.0, bw_a=1.0, x_num=1000,
...
class MaskCTRMemoryCEScoreDist(BaseDetector):
    def __init__(self,
                 hidden_neurons=None,
                 hidden_activation='relu',
                 batch_norm=True,
        ...
        super(MaskCTRMemoryCEScoreDist, self).__init__(contamination=contamination)
            ...
            if epoch >= self.start_pesudo_epoch:
                ...
                if self.cluster_methods == 'SelfDist':
                    ...
                    if epoch == self.start_pesudo_epoch:
                        ...
                        self.model.memory = ClusterMemory(self.hidden_neurons[-1], num_cluster=2, temp=0.01,
        ...
        # for data, data_idx, batch_y_label, batch_sample_ind in train_loader:
            ...
            if (epoch >= self.start_pesudo_epoch) and ("score_distribution_loss" in self.loss_fn_name):
                ...
                if pseudo_outlier_mask[0].size(0) > 0:
                    ...
                    print(norm_weight[pseudo_outlier_mask[0]])
        ...
        with torch.no_grad():
            for data, data_idx, _, _, _ in dataloader:
                ...
                memory_logits[data_idx] = torch.softmax(
                    F.normalize(self.model.memory(infer_res['latent_mask_h'])["logits"], dim=1),
                    dim=1
                ).cpu().numpy()
    ...
    def _process_decision_scores(self, sample_name=None):
        ...
        if sample_name is not None:
            ...
            norm_weight = self.decision_scores_['memory_logits'][norm_labels_ind]
