...
class AdaptiveAE(BaseDetector):
    ...
    def generate_pseudo_label(self, history_info, history_outlier_label, eval_counts):
        avg_gradients = []
        max_gradients = []
        slopes = []
        sample_id = []
        for idx, score_info in history_info.items():
            ...
            avg_gradients.append(
                np.mean(np.gradient(history_info[idx]))
            )
            max_gradients.append(
                np.max(np.gradient(history_info[idx]))
            )
            X = np.arange(len(history_info[idx])).reshape(-1, 1)
            model = LinearRegression().fit(X, history_info[idx])
            slopes.append(model.coef_[0])
        X_features = np.array([avg_gradients, max_gradients, slopes]).T
        # 标准化
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(len(train_loader)):
                ...
                if ("mem_loss" in self.loss_fn_name) and (epoch > self.update_epoch):
                    ...
                    mem_loss.append(memory_res['CE_loss'].item())
            ...
            if "mem_loss" in self.loss_fn_name or "CLS" in self.loss_fn_name:
                ...
                with torch.no_grad():
                    ...
                    if "mem_loss" in self.loss_fn_name:
                        if epoch >= self.start_pesudo_epoch and epoch <= self.update_epoch:
                            print("===========> starting collecting pseudo label every  epochs <==================")
                            ...
                            if epoch == self.update_epoch:
                                ...
                                # generate new dataset and calculate cluster centers, c
                                # luster_features:<n_samples+1, features.shape[1]>
                                eval_counts = int(0.5 * (self.update_epoch - self.start_pesudo_epoch))
                                pseudo_y_label= self.generate_pseudo_label(sample_score, history_outlier_label, eval_counts)
                                cluster_features = self.generate_cluster_features(pseudo_y_label, features)
                                # Create hybrid memory
                                self.model.memory = ClusterMemory(
                                    self.hidden_neurons[-1], num_cluster=self.n_samples+1,
                                ...
                                pseudo_labeled_dataset = PyODDataset(
                                    ...
                                    pseudo_y=np.expand_dims((pseudo_y_label), axis=1)
                                )
