...
class DiversityMask(nn.Module):
    ...
    def forward(self,z):
        ...
        loss_tensor = torch.log(1+trans_matrix) * scale
...
class Autoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        mid_x_feat = self.encoder(x_mask)
...
class MLM_AE(BaseDetector):
    def __init__(self,
                 ...
                 kl_loss_weight=0.05,
                 device=None):
        super(MLM_AE, self).__init__(contamination=contamination)
        ...
        if "MSE" in self.loss_fn_name:
            self.loss_fn = MSELossFunction(mask_num)
        if "DIV" in self.loss_fn_name:
            self.loss_div_fn = DiversityMask()
        self.score_function = ScoreFunction(mask_num)
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(len(train_loader)):
                ...
                if "MSE" in self.loss_fn_name:
                    ...
                    mean_mse = self.loss_fn(data, model_output['X_hat'])
                    mse_loss_list.append(mean_mse.item())
                    loss = mean_mse

                if "DIV" in self.loss_fn_name:
                    divloss = self.loss_div_fn(model_output['masks'])
                    div_loss_list.append(divloss.item())

                    loss = loss + self.lambda_v*divloss
    ...
    def decision_function(self, X):
        ...
        with torch.no_grad():
            for data, data_idx, _, _, data_mask_fill in dataloader:
                ...
                # np_feat.append(model_res['laten_vec'])
                # cls_list.append(model_res['class_res'])
                batch_dist = self.score_function(data_cuda, model_res['X_hat'])


                # this is the outlier score
                outlier_scores[data_idx] = batch_dist.squeeze().cpu().numpy()
                if ("mem_loss" in self.loss_fn_name):
