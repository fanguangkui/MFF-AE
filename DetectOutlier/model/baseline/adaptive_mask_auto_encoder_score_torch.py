...
存在的一个主要问题是，在 伪标签概率 映射时无法 将概率映射到0.5以下
使用的 L2dist 作为映射向量源都是0以上的，没有负值。
"""
...
class PyODDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, X, y=None, mean=None, std=None, max=None, min=None, y_train_label=None
                 ):
...
# https://github.com/Minqi824/Overlap
...
class AdaptiveMaskAEScore(BaseDetector):
    def __init__(self,
                 hidden_neurons=None,
                 hidden_activation='relu',
                 batch_norm=True,
                 ...
                 topk_value=1,
                 device=None):
        super(AdaptiveMaskAEScore, self).__init__(contamination=contamination)
    ...
    def _train_autoencoder(self, train_loader):
        ...
        for epoch in range(self.epochs):
            ...
            for data, data_idx, batch_y_label in train_loader:
                ...
                if (epoch > 30) and ("mask_loss" in self.loss_fn_name):
                    ...
                    mask_loss_res = self.mask_loss(
                        model_outputs['important_feature'],
                        format_data
                    )
                    loss += 1 * mask_loss_res['loss_v']
                    # t1 = batch_y_label[mask_loss_res['recog_mask'].cpu()]
                    # print(f"unable to classify outlier/inlier: {torch.where(t1==1)[0].size(0)}/{t1.size(0)}")

                    # distribution area loss
                if (epoch > 50) and ("score_distribution_loss" in self.loss_fn_name):
                    l2_dist = self.model.important_mask * (format_data-model_outputs['x_feat'])
                    # l2_dist = data-model_outputs['x_feat']
                    l2_norm = torch.norm(l2_dist, p=2, dim=1, keepdim=True)
                    # todo: assign_prob 全部取平均有改进空间
                    assign_prob = torch.mean(
                        torch.sigmoid(l2_dist/l2_norm),
                        dim=1
                    )
