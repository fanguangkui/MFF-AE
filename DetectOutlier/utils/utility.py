...
def get_dist(A, B=None, metric="Euclidean"):
    '''
    :param points_feat: feature_nums, bs, dim
    :param centroid_feat: feature_nums, part, dim
    :return:part
    '''
    if B is None:
        B = A
    ...
    if metric == 'Euclidean':
        ...
        dist_sq = (
                ...
                + torch.sum(B ** 2, dim=-1, keepdim=True).transpose(-1, -2)
                - 2 * torch.matmul(A, B.transpose(-1, -2))
        )
        dist_sq = torch.clamp(dist_sq, min=0.0)
        return dist_sq
    if metric == 'cos':
        A_normed = F.normalize(A, p=2, dim=2)
        B_normed = F.normalize(B, p=2, dim=2)
        cos_simi = torch.bmm(A_normed, B_normed.permute(0, 2, 1))
        cos_simi = torch.clamp(cos_simi, max=1.0)
        return 1-cos_simi


def non_zero_mean_per_column(tensor):
    """
    在二维矩阵的第一个维度（行方向）上计算每列的非零元素均值

    参数:
        tensor (torch.Tensor): 二维输入矩阵

    返回:
        torch.Tensor: 包含每列非零元素均值的向量
    """
    # 创建非零掩码
    mask = tensor != 0

    # 计算每列非零元素的和
    sum_per_col = torch.where(mask, tensor, torch.zeros_like(tensor)).sum(dim=0)

    # 计算每列非零元素的个数
    count_per_col = mask.sum(dim=0).float()

    # 避免除以零错误（将零计数替换为1）
    count_per_col = torch.where(count_per_col == 0,
                                torch.ones_like(count_per_col),
                                count_per_col)

    # 计算每列的非零均值
    return sum_per_col / count_per_col