...
def get_global(X, strategy):
    if strategy == 'min':
        ...
        # Mean Imputation: Fill missing values with column mean
        X = X.fillna(X.mean())
    elif strategy == 'low_random':
        # Low-Random-Value Imputation: Fill with random values below min
        min_value = X.min().min()
        random_values = np.random.uniform(low=min_value * 0.5, high=min_value, size=X.shape)
        X = X.fillna(pd.DataFrame(random_values, index=X.index, columns=X.columns))
        ...
        # KNN Imputation: Fill missing values using K-Nearest Neighbors
        from sklearn.impute import KNNImputer
        imputer = KNNImputer(n_neighbors=5)
        X_imputed = imputer.fit_transform(X)
        X = pd.DataFrame(X_imputed, index=X.index, columns=X.columns)
    else:
        # Default to min value imputation
        min_value = X.min().min()
        X = X.fillna(min_value)
    return X
...
def get_fill_data(sub_sample, sub_sample_ids, fill_strategy):
    ...
    elif fill_strategy == 'pair_mean':
        # Mean Imputation for pair
        sub_sample = sub_sample.fillna(sub_sample.mean())
        sub_sample.dropna(inplace=True, axis=1)
    elif fill_strategy == 'pair_low_random':
        # Low-Random-Value Imputation for pair
        min_value = sub_sample.min().min()
        random_values = np.random.uniform(low=min_value * 0.5, high=min_value, size=sub_sample.shape)
        sub_sample = sub_sample.fillna(pd.DataFrame(random_values, index=sub_sample.index, columns=sub_sample.columns))
        sub_sample.dropna(inplace=True, axis=1)
    elif fill_strategy == 'pair_linear':
        # Linear Interpolation for pair
        sub_sample = sub_sample.interpolate(method='linear', axis=1, limit_direction='both')
        sub_sample = sub_sample.fillna(sub_sample.mean())
        sub_sample.dropna(inplace=True, axis=1)
    elif fill_strategy == 'pair_knn':
        # KNN Imputation for pair
        from sklearn.impute import KNNImputer
        imputer = KNNImputer(n_neighbors=1)
        sub_sample_imputed = imputer.fit_transform(sub_sample)
        sub_sample = pd.DataFrame(sub_sample_imputed, index=sub_sample.index, columns=sub_sample.columns)
        sub_sample.dropna(inplace=True, axis=1)
    else:
...
def data_config(args, model_name=None):
    ...
    args.infer_log_dir = os.path.join(args.logs_dir, 'Inference', args.infer_datafile.split(".csv")[0].split(".gz")[0].split(".npz")[0])
    ...
    if model_name =='SEAOP':
        ...
        if not PathManager.exists(os.path.join(args.logs_dir, "outliers", args.infer_datafile.split(".csv")[0].split(".gz")[0])):
            PathManager.mkdirs(os.path.join(args.logs_dir, "outliers", args.infer_datafile.split(".csv")[0].split(".gz")[0]))
    return args