...
#     with open("../datasets/Raw_data/protential_outlier.json", "r") as file:
#         protential_outlier = json.load(file)
#     all_outlier = []
#     model_cnt = 0
#     for imodel, outlier_info in protential_outlier.items():
#         all_outlier.extend(outlier_info['outlier'])
#         model_cnt += 1
#
#     print(f"length of all outlier {len(set(all_outlier))} which detected by single models {model_cnt}")
#     datas = pd.read_csv("../datasets/Format/HelaGroupsV2.csv", index_col=0)
#     filtered_df = datas[~datas.index.isin(all_outlier)]
#
#     df = filtered_df.replace(0, float('NaN'))
#     df.to_csv('../datasets/Clean/HelaGroupsV2_clean.csv', header=True)
#
#
#
# # if __name__ == '__main__':
...
import pandas as pd
data = pd.read_csv('../datasets/Raw_data/peptides.txt', delimiter='\t')
print(data.shape)
seq_data = data['Sequence']
print(seq_data.shape)
seq_data.to_csv('../datasets/Raw_data/peptides_seq.csv', index=False)